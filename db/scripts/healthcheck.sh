...
LOG_FILE="/var/log/superbrain/healthcheck.log"
...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        log "ERROR: 容器重启失败！"
        [ -n "$ALERT_EMAIL" ] && echo "PostgreSQL容器重启失败，请立即检查！" | mail -s "[ALERT] SuperBrain PG Down" "$ALERT_EMAIL"
        exit 1
    fi
    log "INFO: 容器已成功重启"
fi
...
# 检查 PostgreSQL 连接
if ! docker exec "$CONTAINER_NAME" pg_isready -U postgres -d superbrain > /dev/null 2>&1; then
    log "WARNING: PostgreSQL 无法接受连接"
    exit 1
fi

# 检查扩展是否加载
EXTENSIONS_OK=$(docker exec "$CONTAINER_NAME" psql -U postgres -d superbrain -tAc \
